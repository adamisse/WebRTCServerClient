...
namespace Signaler.Hubs
{
    ...
    public class WebRTCHub : Hub
    {
        ...
        //public Task<string> GetSomeRoom()
        //{
        //    var room = _roomManager.GetAll().FirstOrDefault();
        //    await Clients.All.SendAsync("RoomCandidate", room.Id);
        ...
        public async Task JoinRoom(string roomName)
        {
            ...
            //var room = _roomManager.GetAll().FirstOrDefault();
            var user = _userManager.GetAll().Where(u => u.ConnectionId == Context.ConnectionId).First();
            room.AddUser(user);
            user.Room = room;
            user.IsInCall = true;
        ...
        {
            var user = _userManager.GetAll().Where(u => u.ConnectionId == Context.ConnectionId).Single();//problemas aqui?
            //var room = _roomManager.GetAll().Where(r => r.Id == user.Room.Id).Single();
